...
impl BenchmarkResult {
    pub fn human_readable_format(&self) -> impl Fn(Duration) -> String {
        ...
        .map(|d| {
            scales
                ...
                .unwrap_or_else(|| {
                    scales
                        .last()
                        .unwrap_or_else(|| unreachable!("The scales array should not be empty"))
                })
        })
        ...
        // The f64 uses 53 bits of precision, which is already large enough to
        // hold more than 100 days in nanoseconds, which should be more
        // than enough for any benchmark.
...
where
    ...
    S: AsRef<str>,
{
    // Cold run to get a sense of how long a single run takes, which will be
    // used to determine how many iterations we can run in the given time
    // limit.
    let single_run = measure_once(&mut f);
    let iterations = time_limit.as_nanos() / single_run.as_nanos().max(1);
